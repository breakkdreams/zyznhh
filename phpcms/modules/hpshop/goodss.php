...
class goods extends admin {
	...
	*/
	public function goodsadd(){
		...
		if($_POST['dosubmit']){
			...
			$attrinfo = array2string($_POST['goods_attr']);
			$attrinfos=[];
			$arr=[];
			if($_POST['goods_attrs']){
				foreach ($_POST['goods_attrs'] as $k => $v) {
					foreach ($v as $ks => $vs) {
						foreach ($vs as $k1 => $v1) {
							if(empty($v1) || in_array($k1,$arr)){
								if($ks=='name'){
									$arr[]=$k1;
								}else{
									if(!$attrinfos[$k][$k1]['name']){
										unset($attrinfos[$k][$k1]);
									}else{
                                        $attrinfos[$k][$k1][$ks] = 0;
                                    }
								}
			...
			// dump($attrinfos,true);
			$attrinfos = array2string($attrinfos);
	...
	*/
	public function goodsedit(){
		...
		if($_POST['dosubmit']){
			...
			$ainfo = string2array($info['attrinfo']);
			$ainfos = string2array($info['attrinfos']);
			...
			$atinfo = $this->goodstype_db->get_one(array('id'=>$info['type_id']));
			$atinfos = string2array($atinfo['type_content']);
			$attrarr = [];
			foreach ($atinfos as $k => $v) {
				if($v['type']==2){
					$attribute = explode(',', $v['values']);
	...
	*/
	...
	public function getattr() {
		...
		$id=$_POST['tid'];
		$info = $this->goodsattr_db->select(array('goodstypeid'=>$id,'attrtype'=>1),'*','','sort ASC');
		$infos = $this->goodsattr_db->select(array('goodstypeid'=>$id,'attrtype'=>0),'*','','sort ASC');
		$num = count($info);
		$ninfo = $this->newattr($info,$num);
		echo json_encode($ninfo,JSON_UNESCAPED_UNICODE);
	}	
	 ...
	 * @param $num 运行次数
	 */
	public function newattr($arr,$num,$time=0) {
		$data = [];
		...
		$time++;
		if ( $num > 0 ) {
			$data=$this->newattr($arr,$num,$time);
		}
	...
	public function zyconfig()
	{
		...
		$info=$this->zyconfig_db->listinfo('',$order,$page,20);
