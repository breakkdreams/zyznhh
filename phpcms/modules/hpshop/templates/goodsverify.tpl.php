...
<div class="content-menu ib-a blue line-x" id="button">
   
    <a href="?m=hpshop&c=goods&a=goodsverify" <?php if($_GET['level']==NULL){?> class="on" <?php }?>
><em>待审核商品</em></a><span>|</span>
    <a href="?m=hpshop&c=goods&a=goodsverify&level=1" <?php if($_GET['level']=='1'){?> class="on" <?php }?>><em>被驳回商品</em></a>
...
<input type="hidden" value="goodsverify" name="a">
...
<form name="myform" id="myform" action="?m=hpshop&c=goods&a=goodsagree" method="post" onsubmit="checkuid();return false;" >
<div class="table-list">
...
?>   
	...
	<td align="center">
        <a href="javascript:void(0);" class="btn btn-info btn-sm" onclick="view('<?php echo $info['id']?>')"><?php echo L('查看信息')?></a>
        <a href="javascript:confirmurl('?m=hpshop&c=goods&a=goodsagree&id=<?php echo $info['id']?>', '<?php echo L('确定此商品审核通过')?>')" class="btn btn-success btn-sm"><?php echo L('审核通过')?></a>
        <?php if(!$_GET['level']){?>
    	<a href="javascript:confirmurl('?m=hpshop&c=goods&a=goodsdisagree&id=<?php echo $info['id']?>', '<?php echo L('确定驳回该商品吗')?>')" class="btn btn-danger btn-sm"><?php echo L('违规驳回')?></a>
        <?php }?>
	</td>
	</tr>
<?php 
...
<div style="overflow: hidden;">
    <div class="btn" style=" width: 100%; text-align: left; margin-left: 0; "> 
        ...
        <input type="submit" class="btn btn-success btn-sm" name="dosubmit" value="批量审核" onclick="return confirm('<?php echo L('确定选中的商品通过审核吗！')?>')"/>
        <?php if(!$_GET['level']){?>
        <input id='submit' type="submit" class="btn btn-danger btn-sm" onClick="document.myform.action='?m=hpshop&c=goods&a=goodsdisagree'" value="批量驳回"/>
        <?php }?>
    </div>
</div>
...
function view(id) {
    window.top.art.dialog({
        id:'view',
        iframe:'?m=hpshop&c=goods&a=goodsview&id='+id,
        title:'查看商品信息',
        width:'1200',
        height:'700',
        lock:true
...
function checkuid() {
    ...
    if(ids=='') {
        ...
        var url = "?m=hpshop&c=goods&a=goodsdisagree";  
        var z = $("#myform").attr("action");
        if(url==z){
            var mymessage=confirm("确定驳回选中的商品吗！");
            if(mymessage==true){
                myform.submit();
            }else{
