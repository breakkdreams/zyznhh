...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#bname").formValidator({onshow:"请填写品牌名称",onfocus:"请填写品牌名称"}).inputValidator({min:1,max:999,onerror:"品牌名称不能为空"}).ajaxValidator({
	    ...
	    type : "get",
		url : "",
		data :"m=hpshop&c=goods&a=checkbrand_ajax&id=<?php echo $info['id'];?>",
		datatype : "html",
		async:'false',
...
<div class="common-form">
	<div id="div_setting_2" class="contentList">
		...
		<table width="100%" class="table_form">
			<tbody>
				...
				<tr>
					<th width="125">品牌名称</th>  
					<td><input type="text" name="bname" id="bname" class="input-text" value="<?php echo $info['brandname'];?>" required=""><span id="balance"></span></input></td>
				</tr>
				<tr>
					<th>品牌LOGO</th>  
					<td>
						<div class='upload-pic img-wrap'><input type='hidden' name='thumb' id='thumb' value='<?php echo $info['brandimg'];?>'>
	...
	d.uploadfile();return false;}, function(){window.top.art.dialog({id:'crop'}).close()});
                ...
                <tr>
					<th>品牌状态</th>  
					<td>
						<input type="radio" name="status" id="pt" value="1" <?php if ($info['status']==1) {?>checked="checked"<?php }?>>
						<label for="pt">启用</label>

				        <input type="radio" name="status" id="fxs" value="2" <?php if ($info['status']==2) {?>checked="checked"<?php }?>>
						<label for="fxs">关闭</label>
					</td>
				</tr>
