...
class space extends admin {
	...
	function __construct() {
		parent::__construct();
		$setting = new_html_special_chars(getcache('advertisement', 'commons'));
		$this->M = $setting[$this->get_siteid()];
		$this->db = pc_base::load_model('advertisement_space_model');
	}
	...
	public function init() {
		...
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=advertisement&c=space&a=add\', title:\''.L('add_space').'\', width:\'540\', height:\'320\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_space'));
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($spaceid) {
				if ($space['type']=='code') {
					...
					$path = 'advertisement_js/'.$spaceid.'.js';
				}
				$this->db->update(array('path'=>$path), array('siteid'=>$this->get_siteid(), 'spaceid'=>$spaceid));
				showmessage(L('added_successful'), '?m=advertisement&c=space', '', 'add');
			}
		} else {
			$TYPES = $this->template_type();
			$advertisement_template = getcache('advertisement_template_'.$this->get_siteid(), 'commons');
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($space['type']=='code') {
				...
				$space['path'] = 'advertisement_js/'.$_GET['spaceid'].'.js';
			}
			if (isset($_POST['old_type']) && $_POST['old_type']!=$space['type']) {
				$advertisement_db = pc_base::load_model('advertisement_model');
				$advertisement_db->delete(array('spaceid'=>$_GET['spaceid']));
				$space['items'] = 0;
			}
			if ($this->db->update($space, array('spaceid'=>$_GET['spaceid']))) showmessage(L('edited_successful'), '?m=advertisement&c=space', '', 'testIframe'.$_GET['spaceid']);
		} else {
	...
	public function public_preview() {
		if (is_numeric($_GET['spaceid'])) {
			...
			if ($r['type']=='code') {
				$db = pc_base::load_model('advertisement_model');
	...
	private function template_type() {
		pc_base::load_app_func('global','advertisement');
		return get_types();
	}
	...
	public function setting() {
		if (isset($_POST['dosubmit'])) {
			$setting = getcache('advertisement', 'commons');
			$setting[$this->get_siteid()] = $_POST['setting'];
			setcache('advertisement', $setting, 'commons'); //设置缓存
    ...
    public function poster_template() {
        ...
        $templatedir = PC_PATH.$tpl_root.pc_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.'advertisement'.DIRECTORY_SEPARATOR;
        $poster_template = getcache('advertisement_template_'.get_siteid(), 'commons');
        ...
        $big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=advertisement&c=space&a=add\', title:\''.L('add_space').'\', width:\'540\', height:\'320\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_space'));
        include $this->admin_tpl('advertisement_template');
    }
	...
	public function public_tempate_del() {
		...
		$advertisement_template = getcache('advertisement_template_'.$siteid, 'commons');
		if ($advertisement_template[$_GET['id']]) {
			unset($advertisement_template[$_GET['id']]);
            setcache('advertisement_template_'.$siteid, $advertisement_template, 'commons');
            showmessage(L('operation_success'), HTTP_REFERER);
		}else{
            showmessage(L('广告模板不存在'), HTTP_REFERER);
        }
	}
	/**
	 * 配置模板
	...
	public function public_tempate_setting() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$advertisement_template[$_POST['template']] = $_POST['info'];
			setcache('advertisement_template_'.$siteid, $advertisement_template, 'commons');
			...
			if ($advertisement_template[$template]) {
				$info = $advertisement_template[$template];
	...
	public function create_js($page = 0) {
		...
		$page++;
		if ($page>$pages) {
			showmessage(L('update_js_success'),HTTP_REFERER);
		} else {
			showmessage(L('update_js').'<font style="color:red">'.($page-1).'/'.$pages.'</font>', '?m=advertisement&c=space&a=create_js&page='.$page.'&pages='.$pages);
		}
	}
