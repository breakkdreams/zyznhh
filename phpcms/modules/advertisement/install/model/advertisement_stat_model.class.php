...
class advertisement_stat_model extends model {
	function __construct() {
		$this->db_config = pc_base::load_config('database');
		$this->db_setting = 'default';
		$this->table_name = 'advertisement_'.date('Ym');
		parent::__construct();
	...
	private function change_table($tablename = '') {
		if ($tablename) $this->table_name = $this->db_tablepre.'advertisement_'.$tablename;
	}
	...
	public function get_list($year = '') {
		...
		for($y=$diff;$y>0;$y--) {
			...
			if($value<'201006' || !$this->db->table_exists($this->db_tablepre.'advertisement_'.$value)) break;
