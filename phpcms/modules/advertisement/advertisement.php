...
class advertisement extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->s_db = pc_base::load_model('advertisement_space_model');
	...
	public function init() {
		...
		include $this->admin_tpl('advertisement_list');
	}
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			$advertisement = $this->check($_POST['advertisement']);
			$setting = $this->check_setting($_POST['setting'], $advertisement['type']);
			$advertisement['siteid'] = $this->get_siteid();
			$advertisement['setting'] = array2string($setting);
			$advertisement['addtime'] = SYS_TIME;
			$id = $this->db->insert($advertisement, true);
			if ($id) {
				$this->s_db->update(array('items'=>'+=1'), array('spaceid'=>$advertisement['spaceid'], 'siteid'=>$this->get_siteid()));
				$this->create_js($advertisement['spaceid']);
				...
				if (pc_base::load_config('system','attachment_stat')) {
					...
					$this->attachment_db->api_update($imgs, 'advertisement-'.$id, 1);
				}
				showmessage(L('add_ads_success'), HTTP_REFERER);
			} else {
		...
		include $this->admin_tpl('advertisement_add');
	}
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$setting = $this->check_setting($_POST['setting'], $advertisement['type']);
			$advertisement['setting'] = array2string($setting);
			$this->db->update($advertisement, array('id'=>$_GET['id'], 'siteid'=>$this->get_siteid()));
			...
			if(pc_base::load_config('system','attachment_stat')) {
				$this->attachment_db = pc_base::load_model('attachment_model');
				$this->attachment_db->api_update($imgs, 'advertisement-'.$_GET['id'], 1);
			}
			showmessage(L('edit_ads_success'), 'index.php?m=advertisement&c=advertisement&a=init&spaceid='.$_GET['spaceid']);
		} else {
			...
			pc_base::load_sys_class('form', '', 0);
			include $this->admin_tpl('advertisement_edit');
		}
	}
	...
	public function stat() {
		 ...
		 */
		$sdb = pc_base::load_model('advertisement_stat_model'); //调用广告统计的数据模型
		...
		$show_header = true;
		unset($r);
		include $this->admin_tpl('advertisement_stat');
	}
	...
	private function get_setting($type) {
		$data = $advertisement_template = array();
		...
		if (is_array($advertisement_template) && !empty($advertisement_template)) {
			$data = $advertisement_template[$type];
		} else {
			switch($type) {
				case 'banner':
	 ...
	 * ajax检查广告名的合法性
	 */
	public function public_check_advertisement() {
		if (!$_GET['name']) exit(0);
		if (CHARSET=='gbk') {
