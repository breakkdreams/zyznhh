...
<div class="subnav">
    ...
    <?php echo admin::submenu($_GET['menuid'],$big_menu); ?><span>|</span><a href="javascript:window.top.art.dialog({id:'setting',iframe:'?m=advertisement&c=space&a=setting', title:'<?php echo L('module_setting')?>', width:'540', height:'320'}, function(){var d = window.top.art.dialog({id:'setting'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'setting'}).close()});void(0);"><em><?php echo L('module_setting')?></em></a>
    </div>
</div>

<form method="post" action="?m=advertisement&c=advertisement&a=add" id="myform">
<table class="table_form" width="100%" cellspacing="0">
<tbody>
	<tr>
		<th width="100"><?php echo L('poster_title')?>：</th>
		<td><input name="advertisement[name]" id="name" class="input-text" type="text" size="25"></td>
	</tr>
	<tr>
		<th><?php echo L('for_postion')?>：</th>
		<td><?php if($spaceid) { echo '<b style="color:#F60;">',$sinfo['name'],'</b>&nbsp;[',$TYPES[$sinfo['type']],']<input type="hidden" name="advertisement[spaceid]" value="'.$spaceid.'">'; } else {?><select name="advertisement[spaceid]" id="spaceid" onchange="AdsType(this.value)">
	...
	<tr>
        ...
        <td valign="top" colspan="2"><?php echo form::select($setting['type'], '', 'name="advertisement[type]" id="type" onchange="AdsType(this.value)"', $default);?>
        </td>
    </tr>
	<tr>
		<th><?php echo L('line_time')?>：</th>
		<td><?php echo form::date('advertisement[startdate]', date('Y-m-d H:i:s', SYS_TIME), 1)?></td>
	</tr>
	<tr>
		<th><?php echo L('down_time')?>：</th>
		<td><?php echo form::date('advertisement[enddate]', '', 1)?></td>
	</tr>
	</tbody>
	</table>
	
	<?php if(array_key_exists('images', $setting['type'])) {?><div class="pad-10" id="imagesdiv" style="display:">
	<fieldset>
  ...
  <tr>
    ...
    <td rowspan="2"><a href="javascript:flashupload('imgurl<?php echo $i;?>_images', '<?php echo L('upload_photo')?>','imgurl<?php echo $i;?>',preview,'1,<?php echo $thisExt?>,1','advertisement', '', '<?php echo $authkey?>');void(0);"><img src="<?php echo IMG_PATH;?>icon/upload-pic.png" id="imgurl<?php echo $i;?>_s" width="105" height="88"></a><input type="hidden" id="imgurl<?php echo $i;?>" name="setting[images][<?php echo $i;?>][imageurl]"></td>
  </tr>
  <tr>
  ...
  <tr>
    <th>背景色：</th>
    <td class="y-bg"><input type="text" class="input-text" name="setting[images][<?php echo $i;?>][color]" id="color<?php echo $i;?>" size="30" /></td>
  </tr>
</table>
<?php } } else {?>
<table width="100%"  class="table_form">
  ...
  <tr>
    ...
    <td rowspan="2"><a href="javascript:flashupload('imgurl_images', '<?php echo L('upload_photo')?>','imgurl',preview,'1,<?php echo $thisExt?>,1','advertisement', '', '<?php echo $authkey?>');void(0);"><img src="<?php echo IMG_PATH;?>icon/upload-pic.png" id="imgurl_s" width="105" height="88"></a><input type="hidden" id="imgurl" name="setting[images][1][imageurl]"></td>
  </tr>
  <tr>
    <th><?php echo L('alt')?>：</th>
  ...
  <tr>
    <th>背景色：</th>
    <td class="y-bg"><input type="text" class="input-text" name="setting[images][1][color]" id="color3" size="30" /></td>
  </tr>
  </tbody>
</table>
<?php } ?>
...
<div class="pad-10" id="flashdiv" style="display:none;">
  ...
  <tr>
    ...
    <td class="y-bg"><input type="button" class="button" onclick="javascript:flashupload('flashurl<?php echo $i;?>_images', '<?php echo L('flash_upload')?>','flashurl<?php echo $i;?>',submit_attachment,'1,<?php echo $thisExt?>,1','advertisement', '', '<?php echo $authkey?>')" value="<?php echo L('flash_upload')?>"></td>
  </tr>
  </tbody>
</table>
<?php } } else {?>
<table width="100%"  class="table_form">
  ...
  <tr>
    ...
    <td class="y-bg"><input type="button" class="button" onclick="javascript:flashupload('flashurl_images', '<?php echo L('flash_upload')?>','flashurl',submit_attachment,'1,<?php echo $thisExt?>,1','advertisement', '', '<?php echo $authkey?>')" value="<?php echo L('flash_upload')?>"></td>
  </tr>
  </tbody>
</table>
<?php } ?>
...
$(document).ready(function(){
	...
	$('#name').formValidator({onshow:"<?php echo L('please_input_name')?>",onfocus:"<?php echo L('name_three_length')?>",oncorrect:"<?php echo L('correct')?>"}).inputValidator({min:6,onerror:"<?php echo L('adsname_no_empty')?>"}).ajaxValidator({type:"get",url:"",data:"m=advertisement&c=advertisement&a=public_check_advertisement",datatype:"html",cached:false,async:'true',success : function(data) {
        if( data == "1" )
		{
            return true;
		}
