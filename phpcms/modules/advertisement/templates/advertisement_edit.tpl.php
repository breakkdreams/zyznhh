...
<form method="post" action="?m=advertisement&c=advertisement&a=edit&id=<?php echo $_GET['id']?>&spaceid=<?php echo $info['spaceid']?>" id="myform">
<table class="table_form" width="100%" cellspacing="0">
<tbody>
	<tr>
		<th width="100"><?php echo L('poster_title')?>：</th>
		<td><input name="advertisement[name]" id="name" value="<?php echo $info['name']?>" class="input-text" type="text" size="25"></td>
	</tr>
	<tr>
	...
	<tr>
        ...
        <td valign="top" colspan="2"><?php echo form::select($setting['type'], trim($info['type']), 'name="advertisement[type]" id="type" onchange="AdsType(this.value)"', $default);?>
        </td>
    </tr>
	<tr>
		<th><?php echo L('line_time')?>：</th>
		<td><?php echo form::date('advertisement[startdate]', date('Y-m-d H:i:s', $info['startdate']), 1)?></td>
	</tr>
	<tr>
		<th><?php echo L('down_time')?>：</th>
		<td><?php echo form::date('advertisement[enddate]', date('Y-m-d H:i:s', $info['enddate']), 1)?></td>
	</tr>
	</tbody>
	</table>
	
	<?php if(array_key_exists('images', $setting['type'])) {?><div class="pad-10" id="imagesdiv" style="display:<?php if($info['type']=='flash') {?>none;<?php }?>">
	<fieldset>
  ...
  <tr>
    <th>背景色：</th>
    <td class="y-bg"><input type="text" class="input-text" name="setting[images][<?php echo $i;?>][color]" id="color<?php echo $i;?>" value="<?php echo $info['setting'][$i]['color']?>" size="30" /></td>
  </tr>
</table>
<?php } } else {?>
<table width="100%"  class="table_form">
  ...
  <tr>
    ...
    <td rowspan="2"><a href="javascript:flashupload('imgurl_images', '<?php echo L('upload_photo')?>','imgurl',preview,'1,<?php echo $thisExt?>,1','advertisement', '', '<?php echo $authkey?>');void(0);"><img src="<?php echo $info['setting'][1]['imageurl']?>" id="imgurl_s" width="105" height="88" onerror="this.src='<?php echo IMG_PATH;?>nopic.gif'"></a><input type="hidden" id="imgurl" name="setting[images][1][imageurl]" value="<?php echo $info['setting'][1]['imageurl']?>"></td>
  </tr>
  <tr>
    <th><?php echo L('alt')?>：</th>
  ...
  <tr>
    <th>背景色：</th>
    <td class="y-bg"><input type="text" class="input-text" name="setting[images][1][color]" value="<?php echo $info['setting'][1]['color']?>" id="color3" size="30" /></td>
  </tr>
  </tbody>
</table>
<?php } ?>
