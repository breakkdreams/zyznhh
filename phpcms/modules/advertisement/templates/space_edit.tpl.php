...
?>
<form method="post" action="?m=advertisement&c=space&a=edit&spaceid=<?php echo $_GET['spaceid']?>" name="myform" id="myform">
...
function AdsType(adstype) {
	...
	<?php 
			if (is_array($advertisement_template) && !empty($advertisement_template)) {
				$n = 0;
				foreach ($advertisement_template as $key => $p) {
					if ($n==0) {
						echo 'if (adstype==\''.$key.'\') {';
					} else {
...
$(document).ready(function(){
	...
	$("#name").formValidator({onshow:"<?php echo L('please_input_space_name')?>",onfocus:"<?php echo L('spacename_three_length')?>",oncorrect:"<?php echo L('correct')?>"}).inputValidator({min:6,onerror:"<?php echo L('spacename_illegality')?>"}).ajaxValidator({type:"get",url:"",data:"m=advertisement&c=space&a=public_check_space&spaceid=<?php echo $_GET['spaceid']?>",datatype:"html",cached:false,async:'true',success : function(data) {
            if( data == "1" )
			{
                return true;
			}
